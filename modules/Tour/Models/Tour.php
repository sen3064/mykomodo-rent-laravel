...
use App\Models\BravoPriceCalendar;
...
class Tour extends Bookable
{
    ...
    public function getAdminFee($price){
        $en = intval($price);
        $tot = 0;
        if($en>=0 && $en<=25000){
            $tot = $en*(8/100);
    ...
    public function addToCart(Request $request)
    {
        ...
        $total_before_fees = $total;
        $total_buyer_fee = 0;
        $admin_fee = $this->getAdminFee($price);
        if(!$is_private){
            $admin_fee = $admin_fee*$total_guests;
        }
        $total_buyer_fee = $admin_fee;
        //Service Fees for Vendor
        $total_service_fee = 0;
        ...
        $booking = new $this->bookingClass();
        $booking->status = 'draft';
        $booking->object_id = $request->tour_id;
        ...
        if ($check) {
            ...
            return $this->sendSuccess([
                // 'url'          => $booking->getCheckoutUrl(),
                // 'booking_code' => $booking->code,
                'object_data'=>['parent'=>TourParent::find($request->parent_id),'tour'=>$this],
                'booking_data'=>$booking
            ],'Pesanan anda telah di buat, silahkan selesaikan proses pembayaran');
        }
        return $this->sendError(__("Can not check availability"));
    }
    ...
    public static function search(Request $request)
    {
        ...
        if($request->has('tour_date')){
            $check_date = date("Y-m-d", strtotime($request->tour_date));
        }
        
        $parent_tour = TourParent::where('status','publish');
        if (!empty($location_id = $request->query('location_id'))) {
            if($location_id!='0'){
                $parent_tour->where('location_id',$location_id);
            }
        }
        $list = $parent_tour->with(['location:id,name'])->get();
        // dd(DB::getQueryLog());
        ...
        foreach($list as &$row){
            // DB::enableQueryLog();
            if($row->image_id){
                $media = MediaFile::find($row->image_id);
            ...
            $model_Tour->where("bravo_tours.parent_id", $row->id);
    ...
    //            ORDER BY (POW((lon-$lon),2) + POW((lat-$lat),2))";
            ...
            // ])->paginate($limit);
            $row->price = 0;
            $row->variants = $model_Tour->with(['location:id,name','hasWishList'])->get();
            // dd($row->variants);
            foreach($row->variants as &$rv){
                if($rv->image_id){
                    $media = MediaFile::find($rv->image_id);
                    $pre_url = 'https://cdn.mykomodo.kabtour.com/uploads/';
                    $rv->banner = [
                        "original"=> $pre_url.$media->file_path,
                        "200x150"=> $media->file_resize_200 != null ? $pre_url.$media->file_resize_200 : null,
                    ...
                    $rv->price_normal = $rv->price;
                    $rv->price = $rv->price_normal - $rv->discount;
                    if ($rv->price_weekend && $rv->price_weekend > 0) {
                        if (date("D", strtotime($check_date)) === "Sat" || date("D", strtotime($check_date)) === "Sun") {
                            $rv->price = $rv->price_weekend - $rv->discount_weekend;
                        }
                        if (array_key_exists($check_date, $get_holiday)) {
                            $rv->price = $rv->price_weekend - $rv->discount_weekend;
                        }
                    }
                    $getCalendars = BravoPriceCalendar::where(['create_user'=>$rv->create_user,'object_model'=>'tour','price_date'=>$check_date,'status'=>1])->first();
                    if ($getCalendars) {
                        if ($rv->price_holiday && $rv->price_holiday > 0) {
                            $rv->price = $rv->price_holiday - $rv->discount_holiday;
                        }
                    }
                    if($row->price==0){
                        $row->price = $rv->price;
                    }
                    if($rv->price<$row->price){
                        $row->price = $rv->price;
                    }
                    $rv->price = (string)$rv->price;
                }
            }
    ...
    public static function myProducts(Request $request)
    {
        $get_holiday = Http::withOptions([
            'verify' => false,
        ])->get('https://raw.githubusercontent.com/guangrei/Json-Indonesia-holidays/master/calendar.json')->json();
        $check_date = date("Y-m-d", strtotime("+7 Hours"));
        $parent_tour = TourParent::where(['create_user'=>Auth::id(),'status'=>'publish'])->with(['location:id,name'])->get();;
        $list = $parent_tour;
        // $list->variants = [];
        foreach($list as &$row){
            // DB::enableQueryLog();
